class ReviewUsersController < ApplicationController
  ...
  def index
    if params.has_key?(:user_id)
      ...
      @review_users = ReviewUser.where(received_id: @user.id)
     else
  ...
  def create
    ...
    respond_to do |format|
      if @review_user.save
        format.html { redirect_to @review_user, notice: "Review user was successfully created." }
  ...
  def update
    respond_to do |format|
      if @review_user.update(review_user_params)
        format.html { redirect_to @review_user, notice: "Review user was successfully updated." }
  ...
  private
    ...
    def review_user_params
      params.require(:review_user).permit(:text, :rating, :is_a_guest)
    end
end
